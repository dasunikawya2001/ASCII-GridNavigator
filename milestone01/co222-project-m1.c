...
bool isValidMove(char array[][50],int rows,int columns,int i,int j,int prev_i,int prev_j){
    ...
    if (asciiDiff_1 <= 2)
    {
        return true; // Moving formaward and climbing conditions
    }
    else if (asciiDiff_1 <= 3)
    {
        return true; // Moving backward and going down conditions
    }
    return false;
}
...
void Shortest_Path (char array[][50],int rows,int columns,int start_i,int start_j,int end_i,int end_j){
    ...
    while (!isEmpty())
    {
        ...
        if ((present.i == end_i) && (present.j == end_j) )
        {
            printf("The minimum distance from %c to %c is %d" , mapArray[start_i][start_j],mapArray[end_i][end_j]  , minDistance[end_i][end_j]);
            return;
        }
        ...
        for (int m = 0; m < 4 ; m++)
        {
            ...
            if (isValidMove(mapArray,rows , columns , next_i , next_j , present.i , present.j))
            {
    ...
    printf("There is no possible path from %c to %c ", mapArray[start_i][start_j],mapArray[end_i][end_j]);  
} 
...
int main(){
    ...
    Shortest_Path(mapArray,rows,cols,S1,S2,E1,E2);
